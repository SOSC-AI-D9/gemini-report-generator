...
def setup_client_and_tools() -> Tuple['genai.Client', Tool]:
    ...
    client = genai.Client(
        vertexai=True,
        project=os.getenv("GOOGLE_CLOUD_PROJECT","myagent-qitv"),
...
def table_of_contents_prompt(
    ...
    """    
    try:
        logger.info(f"[Generate] Generating Table of Contents with model: {model_id}")
        log_to_request_file(request_id, "generating", f"[Generate] Generating Table of Contents with model: {model_id}")
        ...
        if not isinstance(report_config, dict):
            error_msg = f"ERROR Invalid config type: {type(report_config).__name__}, expected dict"
        ...
        for key in required_keys:
            if key not in report_config:
                error_msg = f"ERROR Missing required config key: {key}"
                logger.error(error_msg)
                log_to_request_file(request_id, "error", error_msg)
        ...
        try:
            ...
            error_msg = f"ERROR Error getting credit card product type: {str(e)}"
            logger.error(error_msg)
            log_to_request_file(request_id, "error", error_msg)
        ...
        try:
            ...
            error_msg = f"ERROR Error creating user prompt: {str(e)}"
            logger.error(error_msg)
            log_to_request_file(request_id, "error", error_msg)
            raise
        ...
        try:
            ...
            error_msg = f"ERROR Error adding user prompt to contents: {str(e)}"
            logger.error(error_msg)
            log_to_request_file(request_id, "error", error_msg)
            raise
        ...
        try:
            ...
            error_msg = f"ERROR Error generating content: {error_details}"
            logger.error(error_msg)
            log_to_request_file(request_id, "error", error_msg)
            raise
        ...
        try:
            text = response.text
            if not text:
                error_msg = "ERROR Empty response text from model"
                logger.error(error_msg)
                log_to_request_file(request_id, "error", error_msg)
            ...
            if not hasattr(response, 'text'):
                error_msg = f"ERROR Response object has no 'text' attribute: {response}"
                logger.error(error_msg)
                log_to_request_file(request_id, "error", error_msg)
            else:
                error_msg = f"ERROR Error extracting text from response: {str(e)}"
                logger.error(error_msg)
                log_to_request_file(request_id, "error", error_msg)
            raise
        ...
        try:
            ...
            error_msg = f"ERROR Error adding model response to contents: {str(e)}"
            logger.error(error_msg)
            log_to_request_file(request_id, "error", error_msg)
            raise
            
        logger.info("OK Table of Contents generated successfully")
        log_to_request_file(request_id, "generating", "OK Table of Contents generated successfully")
        ...
        error_msg = f"ERROR Critical error in table_of_contents_prompt: {error_details}"
        logger.error(error_msg)
        log_to_request_file(request_id, "error", error_msg)
        raise
...
def extract_table_of_contents(
    ...
    """
    try:
        logger.info("[Extract] Extracting main sections from Table of Contents...")
        log_to_request_file(request_id, "generating", "[Extract] Extracting main sections from Table of Contents...")
        ...
        if not isinstance(context_user_prompt, Part):
            error_msg = f"ERROR Invalid context_user_prompt type: {type(context_user_prompt).__name__}, expected Part"
            logger.error(error_msg)
            log_to_request_file(request_id, "error", error_msg)
            raise TypeError(error_msg)
            
        if not isinstance(context_text, str) or not context_text.strip():
            error_msg = f"ERROR Invalid context_text: {'empty' if not context_text.strip() else type(context_text).__name__}"
            logger.error(error_msg)
            log_to_request_file(request_id, "error", error_msg)
            raise ValueError(error_msg)
        ...
        try:
            ...
            error_msg = f"ERROR Error creating extraction prompt: {str(e)}"
            logger.error(error_msg)
            log_to_request_file(request_id, "error", error_msg)
            raise
        ...
        try:
            ...
            error_msg = f"ERROR Error creating contents array for extraction: {str(e)}"
            logger.error(error_msg)
            log_to_request_file(request_id, "error", error_msg)
            raise
        ...
        try:
            ...
            error_msg = f"ERROR Error creating extraction configuration: {str(e)}"
            logger.error(error_msg)
            log_to_request_file(request_id, "error", error_msg)
            raise
        ...
        try:
            ...
            error_msg = f"ERROR Error in TOC extraction API call: {error_details}"
            logger.error(error_msg)
            log_to_request_file(request_id, "error", error_msg)
            raise
        ...
        try:
            text = response.text
            if not text:
                error_msg = "ERROR Empty extraction response text"
                logger.error(error_msg)
                log_to_request_file(request_id, "error", error_msg)
                raise ValueError(error_msg)
            ...
            if not ("TITLE:" in text and "SECTIONS:" in text):
                warning_msg = "WARNING Extraction response may not be in the expected format"
            ...
            if not hasattr(response, 'text'):
                error_msg = f"ERROR Extraction response object has no 'text' attribute: {response}"
                logger.error(error_msg)
                log_to_request_file(request_id, "error", error_msg)
            else:
                error_msg = f"ERROR Error extracting text from extraction response: {str(e)}"
                logger.error(error_msg)
                log_to_request_file(request_id, "error", error_msg)
            raise
            
        logger.info("OK Sections extracted successfully")
        log_to_request_file(request_id, "generating", "OK Sections extracted successfully")
        return user_prompt, text, response
        ...
        error_msg = f"ERROR Critical error in extract_table_of_contents: {error_details}"
        logger.error(error_msg)
        log_to_request_file(request_id, "error", error_msg)
        raise
